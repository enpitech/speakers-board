...
export type SocialNetwork = "twitter" | "linkedin" | "facebook" | "instagram" | "youtube" | "github" | "tiktok"
...
interface SocialNetworkInputProps {
  ...
  onChange: (value: { network: SocialNetwork; username: string }) => void
  className?: string
}
...
export function SocialNetworkInput({ value, onChange, className }: SocialNetworkInputProps) {
  ...
  const selectedNetwork = socialNetworks.find((network) => network.value === value.network) || socialNetworks[0]
    ...
    if (buttonRef.current) {
      ...
      const top =
        spaceBelow < dropdownHeight
          ...
          : rect.bottom + scrollTop + 5 // Position below
  ...
  return (
    <div className={cn("flex w-full flex-col gap-2 sm:flex-row", className)}>
            ...
            {socialNetworks.map((network) => (
              <button
                ...
                className={cn(
                  ...
                  network.value === value.network ? "bg-background-1/20" : "",
                )}
              >
                {network.icon}
      ...
      <input
        ...
        placeholder={`Enter your ${selectedNetwork.label} username`}
